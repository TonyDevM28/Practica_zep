/**
 * This file was auto-generated by Fern from our API Definition.
 */
import * as core from "./core/index.mjs";
import { mergeHeaders } from "./core/headers.mjs";
import { UserGroup } from "./api/resources/userGroup/client/Client.mjs";
import { Batch } from "./api/resources/batch/client/Client.mjs";
import { Context } from "./api/resources/context/client/Client.mjs";
import { Graph } from "./api/resources/graph/client/Client.mjs";
import { Project } from "./api/resources/project/client/Client.mjs";
import { Task } from "./api/resources/task/client/Client.mjs";
import { Thread } from "./api/resources/thread/client/Client.mjs";
import { User } from "./api/resources/user/client/Client.mjs";
export class ZepClient {
    constructor(_options = {}) {
        this._options = Object.assign(Object.assign({}, _options), { headers: mergeHeaders({
                "X-Fern-Language": "JavaScript",
                "X-Fern-SDK-Name": "zep-cloud",
                "X-Fern-SDK-Version": "3.28.0",
                "User-Agent": "zep-cloud/3.28.0",
                "X-Fern-Runtime": core.RUNTIME.type,
                "X-Fern-Runtime-Version": core.RUNTIME.version,
            }, _options === null || _options === void 0 ? void 0 : _options.headers) });
    }
    get userGroup() {
        var _a;
        return ((_a = this._userGroup) !== null && _a !== void 0 ? _a : (this._userGroup = new UserGroup(this._options)));
    }
    get batch() {
        var _a;
        return ((_a = this._batch) !== null && _a !== void 0 ? _a : (this._batch = new Batch(this._options)));
    }
    get context() {
        var _a;
        return ((_a = this._context) !== null && _a !== void 0 ? _a : (this._context = new Context(this._options)));
    }
    get graph() {
        var _a;
        return ((_a = this._graph) !== null && _a !== void 0 ? _a : (this._graph = new Graph(this._options)));
    }
    get project() {
        var _a;
        return ((_a = this._project) !== null && _a !== void 0 ? _a : (this._project = new Project(this._options)));
    }
    get task() {
        var _a;
        return ((_a = this._task) !== null && _a !== void 0 ? _a : (this._task = new Task(this._options)));
    }
    get thread() {
        var _a;
        return ((_a = this._thread) !== null && _a !== void 0 ? _a : (this._thread = new Thread(this._options)));
    }
    get user() {
        var _a;
        return ((_a = this._user) !== null && _a !== void 0 ? _a : (this._user = new User(this._options)));
    }
}
